...
- **PR-based flow:** `/sync` creates a GitHub PR via `gh pr create` (idempotent — reuses existing PR). Codex review is posted as a PR comment using a `<!-- codex-review -->` marker for upsert. `/merge` uses `gh pr merge --merge --delete-branch` instead of raw git merge/push/delete. All via `github.ts` standalone functions using `gh` CLI (auto-detects repo from `.git/config`). `GH_PROMPT_DISABLED=1` prevents TTY prompts.
- **Code review via Codex CLI:** `scripts/review-cloud.sh` (local) and `/review` (Telegram) run `codex review --base main`. Review is posted as PR comment if a PR exists, plus shown inline in Telegram. Codex failure reasons (auth expired, rate limit, timeout) are reported instead of terse "Review failed."
- **Mobile merge flow:** From Telegram: `/sync` (push + create PR) → `/review cloud/<branch>` (Codex reviews, posts to PR) → `/merge cloud/<branch>` (merges PR on GitHub, syncs local). No local machine access required.
